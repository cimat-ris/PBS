...
public:
	...
	// Test if obstacle at location loc
	inline bool isObstacle(int loc) const { return map_description[loc]; }
	// Test if move from curr to next is valid
	inline bool validMove(int curr, int next) const;
	// 
	list<int> getNeighbors(int curr) const;

	// Coordinates into single location index 
	inline int linearizeCoordinate(int row, int col) const { return ( this->num_of_cols * row + col); }
	// Single location index into coordinates
	...
	// Manhattan distance between two locations specified by their location indices
	...
	// Manhattan distance between two locations specified by their coordinates
	inline int getManhattanDistance(const pair<int, int>& loc1, const pair<int, int>& loc2) const {
		return abs(loc1.first - loc2.first) + abs(loc1.second - loc2.second);
	}

	// Degree of a location (number of non-obstacle neighbors)
	int getDegree(int loc) const {
		assert(loc >= 0 && loc < map_size && !map_description[loc]);
		int degree = 0;
		if (0 <= loc - num_of_cols && !map_description[loc - num_of_cols])
			degree++;
		if (loc + num_of_cols < map_size && !map_description[loc + num_of_cols])
			degree++;
		if (loc % num_of_cols > 0 && !map_description[loc - 1])
			degree++;
		if (loc % num_of_cols < num_of_cols - 1 && !map_description[loc + 1])
			degree++;
		return degree;
	}
...
private:
	  vector<bool> map_description;
	  string map_fname;
	  string agent_fname;

	  int num_of_agents;
	  // Describes the start and goal locations of all agents
